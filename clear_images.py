...
image_df = pd.read_csv('raw_images.csv')
...
template_paths = image_df[image_df.isTemplate].path
template_hashes = get_hashes(template_paths)
duplicates_index = get_duplicates(template_hashes, template_hashes)[1:]
names_to_drop = image_df.iloc[template_paths.iloc[duplicates_index].index].name.to_list()
image_df = image_df.query(f'not name in {names_to_drop}')
image_df.to_csv(Path.cwd()/'images.csv', encoding='utf-8', index=False)
