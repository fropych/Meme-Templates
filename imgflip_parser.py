...
async def proccess_page(session, html, image_data, path_to_save):
    ...
    for preview_img in soup_main.find_all('img', {'class':'shadow'}):
        ...
        if not parent.parent.find('div', {'class': 'mt-animated-label'}):
            ...
            if template_name in unique_filenames: continue
            unique_filenames.add(template_name)
            template_img_url = soup_sub.find('a', {'class':'meme-link'}).find('img').get('src')
...
async def parse():
    async with aiohttp.ClientSession() as session:
        path_to_save = Path.cwd()/'raw_images'
        path_to_save.mkdir(parents=True, exist_ok=True)
        
        tasks = []
        image_data = []
        for page in range(30):
            async with session.get(f'{url_base}/memetemplates?sort=top-all-time&page={page}') as response:
                html = await response.text()
            task = asyncio.create_task(proccess_page(session, html, image_data, path_to_save))
            tasks.append(task)
...
async def main(create_csv=True):
    image_data = await parse()
    
    if not create_csv: return
    df = pd.DataFrame(image_data)
    df.to_csv(Path.cwd()/'raw_images.csv',  encoding='utf-8', index=False)
